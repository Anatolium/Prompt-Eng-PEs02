...
prompt = PromptTemplate(
   ...
   Список обязательных проверок рассуждений:
   - Наличие обязательных полей: Название, Краткое описание, Полное описание, Преимущества, Характеристики, SEO-ключи.
   - Лимит слов: фактическое количество слов <= 500. (Выдавать число.)
   - Формат/заголовки: соблюдение требуемого формата вывода (заголовки и порядок).
   - Согласованность числовых данных: проверка на внутренние противоречия (пример: «8 часов» vs «до 24 часов без кейса» — объяснить/сопоставить).
   - Соответствие контексту: все ключевые характеристики из {context}, помеченные как "обязательные", должны присутствовать в карточке и иметь совпадающие значения (Bluetooth версия, IPX, время работы и т.п.).
   - Отсутствие повтора/воды: обнаружить дублирующие фразы/предложения (>70% совпадения) и убрать/заменить.
   - Тон и стиль: текст — продающий и информативный (короткая проверка тональности).
   - SEO-проверка: основной ключ из запроса пользователя должен присутствовать в названии или первых 1–2 предложениях; общее число SEO-ключей 3–7; все SEO-ключи перечислены в блоке "SEO-ключевые слова".
   - Юридические/маркетинговые запреты: отсутствие ненаучных/гарантийных заявлений вроде «лечит», «гарантированно 100%», «официально одобрено» и т.п.
   - Единицы измерения: все числовые характеристики снабжены единицами (часы, мА·ч, см, г и т.п.).
