...
prompt = PromptTemplate(
    ...
    При генерации карточки товара необходимо провести пошаговые рассуждения и на каждом шагу делать ключевые заметки на непротиворечивость сгенерированных данных.
    ...
    Необходимо сделать следующие заметки пи генерации карточки
    1. **Категория товара:** (одно слово — какая категория наиболее релевантна запросу)
    2. **Целевая аудитория:** (кто потенциальный покупатель — 1–2 фразы)
    3. **Три ключевых выгоды / УТП:** (основные преимущества, на которых нужно сфокусироваться)
    4. **Ключевые характеристики (4–6 из контекста):** (укажи их, чтобы точно передать данные)
    5. **Возможные пробелы в данных:** (что не указано, но может понадобиться для карточки — например, цвет, материал, размер и т.д.)
